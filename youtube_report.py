...
if __name__ == "__main__":
    ...
    for c in channels:
        ...
        markdown_channel += "## Modelagem de vídeos em alta\n\n"
        for r in c.rewrites_list:
            markdown_channel += f"- {r.rewrite} (Ref: {r.videoTitle})\n"

        markdown_channel += "\n"

        markdown_channel += "## Sugestões baseadas em vídeos alta\n\n"
        for s in filter(lambda x: x.fromType == 'trending', c.sugestions_list):
            markdown_channel += f"- {s.sugestion} (Ref: {s.videoTitle})\n"

        markdown_channel += "\n"

        markdown_channel += "## Sugestões baseadas em vídeos mais recentes\n\n"
        for s in filter(lambda x: x.fromType == 'latest', c.sugestions_list):
            markdown_channel += f"- {s.sugestion} (Ref: {s.videoTitle})\n"
    ...
    # df = create_dataframe(channels)
    # df_channels = df[['channelId', 'channelTitle', 'channelHandle']].drop_duplicates()
